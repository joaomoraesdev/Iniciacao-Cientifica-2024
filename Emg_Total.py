...
# simulate EMG signal - simulacao do sinal
burst1 = np.random.uniform(-65, 65, size=1000) + 0.08 #sinal 1
burst2 = np.random.uniform(-65, 65, size=1000) + 0.08 #sinal 2
quiet = np.random.uniform(-0.05, 0.05, size=500) + 0.08 #periodo vazio; contracoes
emg = np.concatenate([quiet, burst1, quiet, burst2, quiet]) #
time = np.array([i/1000 for i in range(0, len(emg), 1)]) # sampling rate 1000 Hz

# plot EMG signal - plotando no grafico
fig = plt.figure()
plt.plot(time, emg)
plt.xlabel('Time (sec)')
plt.ylabel('EMG (a.u.)')
...
def filteremg(time, emg, low_pass=10, sfreq=1000, high_band=20, low_band=450):
    ...
    str(int(high_band*sfreq)) + '-' + str(int(low_band*sfreq)) + 'Hz')
    plt.plot(time, emg_rectified)
    plt.locator_params(axis='x', nbins=4)
    plt.locator_params(axis='y', nbins=10)
    plt.ylim(-100, 100)
